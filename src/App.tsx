...
function SiteAssessmentScreen({ surveyId }: { surveyId: string }) {
...
function Routed() {
  ...
  const { loadSurvey, survey } = useSurvey();

  const surveyId = route.name === 'job' || route.name === 'products' || route.name === 'assessment' || route.name === 'summary' || route.name === 'window' ? route.surveyId : null;

  useEffect(() => {
    if (surveyId && survey?.id !== surveyId) {
      loadSurvey(surveyId);
    }
  }, [surveyId, survey?.id, loadSurvey]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <main className="flex-1">
        ...
        {route.name === 'job' && <JobScreen key={route.surveyId} surveyId={route.surveyId} />}
        {route.name === 'products' && <ProductsScreen key={route.surveyId} surveyId={route.surveyId} />}
        {route.name === 'assessment' && <SiteAssessmentScreen key={route.surveyId} surveyId={route.surveyId} />}
        {route.name === 'summary' && <SurveySummaryScreen key={route.surveyId} surveyId={route.surveyId} />}
        {route.name === 'window' && (
          <WindowScreen
            key={route.surveyId + route.windowId}
            surveyId={route.surveyId}
            windowId={route.windowId}
            stage={route.stage ?? 'measurements'}
          />
        )}
      </main>
    </div>
